  ...
  useEffect(() => {
    const fetchChefs = async () => {
      const chefsData = await client.fetch(
        `*[_type == "chef"]{ name, position, "image": image.asset->url }`
      );
      setChefs(chefsData);
    };
        ...
        <div className="mx-auto px-4 py-16 md:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            ...
            {chefsToShow.map((chef, index) => (
              <div key={index} className="shadow-md overflow-hidden">
                ...
                />
                <div className="pt-4">
                  <div className="w-[181px] h-[67px] bg-white mt-[-80px] rounded-bl-lg absolute pl-2 ml-2 md:ml-0 ">
                    <h3 className="font-bold text-[#333] text-[18px]">
                      {chef.name}
                    </h3>
          ...
          {!showMore ? (
            <button
              ...
              className="mt-8 block mx-auto px-4 py-2 text-white border-2 border-orange-500 rounded-full hover:bg-orange-600 transition duration-300"
            >
              {" "}
              See More{" "}
            </button>
            ...
            <button
              onClick={handleHide}
              className="mt-8 block mx-auto px-4 py-2 bg-orange-500  text-white rounded-full"
            >
              {" "}
              Hide{" "}
            </button>
